...
function Kalender({ ruang, lapor }) {
  ...
  if (!ruang) return <p className="text-[13px] text-stone-500">Pilih ruang dulu.</p>;
    ...
    <div className="rounded-2xl border border-alba-200 bg-white p-5 shadow-lembut">
      <div className="flex flex-wrap items-center gap-3">
        <label className="text-[12px] font-semibold text-stone-600">
          Dari
          <input type="date" value={dari} onChange={(ev) => setDari(ev.target.value)} className="ml-2 rounded-lg border border-alba-300 px-2.5 py-1.5 text-[12px]" />
        </label>
        <label className="text-[12px] font-semibold text-stone-600">
          Selama
          <select value={hari} onChange={(ev) => setHari(Number(ev.target.value))} className="ml-2 rounded-lg border border-alba-300 px-2.5 py-1.5 text-[12px]">
            <option value={7}>7 hari</option>
            <option value={14}>14 hari</option>
            <option value={31}>31 hari</option>
          </select>
        </label>
        {memuat && <Loader2 size={14} className="animate-spin text-stone-400" />}
      </div>
      ...
      {!kejadian.length && !memuat && (
        <p className="py-8 text-center text-[13px] text-stone-500">Tidak ada kelas, blok, atau booking di rentang ini.</p>
      )}

      <div className="mt-4 space-y-4">
        {Object.keys(perTanggal).map((t) => (
          <div key={t}>
            <p className="text-[12px] font-bold text-stone-500">{t}</p>
    ...
    <section>
      ...
      {baru && (
        <div className="mt-4 grid gap-3 rounded-2xl border border-sewa/30 bg-white p-5 shadow-lembut sm:grid-cols-2">
          ...
          <label className="block">
            <span className="mb-1.5 block text-[12px] font-semibold text-stone-600">Jenis</span>
          ...
          <label className="block">
            <span className="mb-1.5 block text-[12px] font-semibold text-stone-600">Mulai (WIB)</span>
          ...
          <label className="block">
            <span className="mb-1.5 block text-[12px] font-semibold text-stone-600">Selesai (WIB)</span>
          ...
          <label className="block sm:col-span-2">
            <span className="mb-1.5 block text-[12px] font-semibold text-stone-600">Alasan</span>
      ...
      {memuat
        ? <p className="mt-3 text-[13px] text-stone-500">Memuat…</p>
        : (
          <ul className="mt-4 space-y-2">
              ...
              <li key={b.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-alba-200 bg-white px-4 py-3">
                <div>
                  <p className="text-[13px] font-semibold text-stone-700">
                    ...
                    {!b.active && <span className="ml-2 text-[11px] uppercase text-stone-400">nonaktif</span>}
                  </p>
                  <p className="text-[12px] text-stone-500">{jadwalKalimat(b.startAt, b.endAt)} · {b.source}</p>
                </div>
                <button
                  onClick={async () => {
            ...
            {!daftar.length && <li className="text-[13px] text-stone-500">Belum ada blok internal.</li>}
          </ul>
        )}
    </section>
  );
    ...
    <section>
      ...
      {baru && (
        <div className="mt-4 grid gap-3 rounded-2xl border border-sewa/30 bg-white p-5 shadow-lembut sm:grid-cols-2">
          ...
          <label className="block">
            <span className="mb-1.5 block text-[12px] font-semibold text-stone-600">Ruang (kosong = semua ruang)</span>
            <select value={baru.room} onChange={(ev) => setBaru({ ...baru, room: ev.target.value })} className={inputCls}>
      ...
      {memuat
        ...
        : (
          <ul className="mt-4 space-y-2">
            {daftar.map((g) => (
              <li key={g.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-alba-200 bg-white px-4 py-3">
                <div>
                  ...
                  <p className="text-[12px] text-stone-500">{jadwalKalimat(g.startAt, g.endAt)}</p>
                </div>
                <button
                  onClick={async () => {
                    if (!window.confirm(
            ...
            {!daftar.length && <li className="text-[13px] text-stone-500">Belum ada jadwal penjaga.</li>}
          </ul>
        )}
    </section>
  );
