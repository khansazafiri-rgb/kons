...
function Kpi({ label, nilai, nada }) {
  const warna = nada === 'urgen'
    ...
    : nada === 'bahaya'
      ...
      : 'border-slate-200 bg-white text-sewa';
  return (
    <div className={`rounded-2xl border p-4 shadow-lembut ${warna}`}>
      <p className="font-sewa text-2xl font-semibold">{nilai}</p>
      <p className="mt-0.5 text-[12px] font-semibold leading-snug text-slate-600">{label}</p>
    </div>
  );
}
...
function Bukti({ bukti, onVerifikasi, sibuk }) {
  ...
  if (!bukti?.length) {
    return <p className="text-[13px] text-slate-500">Belum ada bukti pembayaran.</p>;
  }
  return (
    <ul className="space-y-2">
      {bukti.map((b) => {
        ...
        return (
          <li key={b.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white p-3">
            {penuh ? (
              <a href={penuh} target="_blank" rel="noreferrer" title="Buka ukuran penuh">
                {/image\//.test(b.mime || '') || !b.mime
                  ? <img src={src} alt="Bukti" className="h-14 w-14 rounded-lg object-cover ring-1 ring-slate-200" />
                  : <span className="grid h-14 w-14 place-items-center rounded-lg bg-slate-100 text-[10px] font-bold text-slate-500">PDF</span>}
              </a>
            ) : <div className="grid h-14 w-14 place-items-center rounded-lg bg-slate-50 text-[10px] text-slate-400">…</div>}

            <div className="min-w-0 flex-1">
              <p className="truncate text-[13px] font-semibold text-slate-700">{b.namaBerkas || 'bukti'}</p>
...
function KartuPesanan({ p, onSegarkan, lapor }) {
  ...
  return (
    <li className="rounded-2xl border border-slate-200 bg-white shadow-lembut">
      <button
        onClick={() => setBuka((b) => !b)}
      ...
      >
        <div className="min-w-0">
          <p className="font-sewa text-[15px] font-semibold text-slate-800">
            {p.kode}
            <span className="ml-2 text-[13px] font-normal text-slate-600">{p.nama}</span>
          </p>
        ...
        <div className="flex items-center gap-3">
          <span className="font-sewa text-[15px] font-semibold text-slate-800">{rupiah(p.total)}</span>
      ...
      {buka && (
        <div className="space-y-5 border-t border-slate-200 p-4">
          ...
          <div>
            <h4 className="text-[12px] font-bold uppercase tracking-wider text-slate-500">Item & jadwal</h4>
            <ul className="mt-2 space-y-2">
              {p.item.map((b) => (
                <li key={b.id} className={`rounded-xl border border-slate-200 px-3 py-2.5 ${b.status === 'DIBATALKAN' ? 'opacity-50' : ''}`}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <p className="text-[13px] font-semibold text-slate-700">
                        {b.nama}{b.jumlah > 1 ? ` ×${b.jumlah}` : ''}
                        {b.status === 'DIBATALKAN' && <span className="ml-2 text-[11px] uppercase text-slate-500">dibatalkan</span>}
                      </p>
                      <p className="text-[12px] text-slate-500">{jadwalKalimat(b.mulai, b.selesai)}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[13px] font-semibold text-slate-700">{rupiah(b.total)}</span>
                      {b.status === 'AKTIF' && p.status !== 'DIBATALKAN' && (
                        <>
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border border-slate-300 px-2.5 py-1.5 text-[11px] font-semibold text-slate-600 hover:border-sewa/50 hover:text-sewa"
                          >
                    ...
                    <div className="mt-3 flex flex-wrap items-end gap-2 border-t border-slate-200 pt-3">
                      <label className="text-[11px] font-semibold text-slate-600">
                        ...
                        <input
                          ...
                          className="mt-1 block rounded-lg border border-slate-300 px-2.5 py-1.5 text-[12px]"
                        />
                      </label>
                      <label className="text-[11px] font-semibold text-slate-600">
                        Selesai (WIB)
          ...
          <div>
            <h4 className="text-[12px] font-bold uppercase tracking-wider text-slate-500">Bukti pembayaran</h4>
            <div className="mt-2">
              <Bukti
                bukti={p.bukti}
                sibuk={sibuk}
            ...
            <label className="mt-2 inline-flex cursor-pointer items-center gap-2 rounded-xl border border-slate-300 px-3.5 py-2 text-[12px] font-semibold text-slate-600 hover:border-sewa/50 hover:text-sewa">
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-slate-200 pt-4">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1.5 rounded-xl border border-slate-300 px-4 py-2.5 text-[12px] font-semibold text-slate-600 hover:border-sewa/50 hover:text-sewa"
            >
            ...
            <select
              ...
              className="rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-[12px] font-semibold text-slate-700 disabled:opacity-50"
            >
          ...
          {teksWa && (
            <pre className="whitespace-pre-wrap rounded-xl border border-slate-200 bg-slate-50 p-4 text-[12px] leading-relaxed text-slate-700">
              {teksWa}
            </pre>
          )}
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <label className="relative min-w-[220px] flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-9 pr-3 text-sm text-slate-700 focus:border-sewa focus:outline-none"
          />
        </label>
        <select
          value={status}
        ...
        <button
          onClick={segarkan}
          className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-600 hover:border-sewa/50 hover:text-sewa"
        >
          Segarkan
        </button>
      </div>
      ...
      {!memuat && daftar.length === 0 && (
        <p className="rounded-2xl border border-dashed border-slate-300 px-6 py-14 text-center text-[13px] text-slate-500">
