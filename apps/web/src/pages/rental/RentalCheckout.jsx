...
function Kolom({ ikon: Ikon, label, children, lebar }) {
  return (
    <label className={`block ${lebar || ''}`}>
      ...
      <span className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 focus-within:border-sewa focus-within:ring-2 focus-within:ring-sewa/20">
        <Ikon size={18} className="mt-0.5 shrink-0 text-slate-400" />
        {children}
      </span>
    </label>
  );
...
const inputCls = 'w-full bg-transparent text-[15px] font-semibold text-slate-900 placeholder:font-normal placeholder:text-slate-400 focus:outline-none';
    ...
    return (
      <RentalLayout konfigurasi={konfigurasi}>
        <div className="mx-auto max-w-md px-6 py-24 text-center">
          <h1 className="text-xl font-extrabold text-slate-900">Keranjangmu kosong</h1>
  ...
  return (
    <RentalLayout konfigurasi={konfigurasi}>
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-8 sm:px-6">
        <Langkah aktif={1} />
        <Link to="/peminjaman/keranjang" className="mt-6 inline-flex items-center gap-1.5 text-sm font-bold text-slate-500 hover:text-slate-900">
          <ArrowLeft size={15} /> Kembali ke keranjang
        </Link>
        <h1 className="mt-2 text-3xl font-extrabold tracking-tight text-slate-900">Data peminjam</h1>
        <p className="mt-1 text-sm text-slate-500">Tidak perlu akun. Admin akan menghubungimu lewat WhatsApp untuk pembayaran.</p>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_22rem] lg:items-start">
          <form onSubmit={kirim} className="space-y-5">
            ...
            <section className="rounded-3xl bg-white p-5 ring-1 ring-slate-200/70 sm:p-6">
              ...
              {konfigurasi.catatanPrivasi && <p className="mt-3 pl-7 text-[12px] leading-relaxed text-slate-500">{konfigurasi.catatanPrivasi}</p>}
              {adaTersimpan && (
                <button
                  type="button"
                  onClick={() => { hapusBiodata(); setAdaTersimpan(false); setSetujuSimpan(false); setForm(KOSONG); }}
                  className="mt-3 inline-flex items-center gap-1.5 pl-7 text-[12px] font-bold text-slate-500 hover:text-rose-600"
                >
            ...
            <button
              ...
              className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-sewa px-5 py-4 text-[16px] font-extrabold text-white hover:bg-sewa-tua disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400"
            >
              {mengirim
          ...
          <aside className="space-y-4 lg:sticky lg:top-24">
            <div className="rounded-3xl bg-white p-5 shadow-angkat ring-1 ring-slate-200/70">
              <h2 className="text-lg font-extrabold text-slate-900">Pesananmu</h2>
              {memeriksa && <p className="mt-2 flex items-center gap-2 text-[13px] text-slate-400"><Loader2 size={14} className="animate-spin" /> Memeriksa ketersediaan…</p>}
                ...
                {isi.map((b, i) => (
                  <li key={`${b.id}-${b.mulai}-${i}`} className="flex gap-3">
                    ...
                    <div className="min-w-0 flex-1 text-[13px]">
                      <p className="font-bold text-slate-900">{b.nama}{b.tipe === 'ALAT' && b.jumlah > 1 ? ` ×${b.jumlah}` : ''}</p>
                      <p className="text-slate-500">{jadwalKalimat(b.mulai, b.selesai)}</p>
                    </div>
                    <p className="shrink-0 text-[13px] font-bold text-slate-800">{hasil?.baris?.[i] ? rupiah(hasil.baris[i].total) : '—'}</p>
                  </li>
                ))}
              </ul>
              <dl className="mt-5 space-y-2 border-t border-slate-100 pt-4 text-[14px]">
                <div className="flex justify-between text-slate-600"><dt>Subtotal</dt><dd>{rupiah(hasil?.subtotal || 0)}</dd></div>
                {(hasil?.biayaTambahan || []).map((b) => (
                  <div key={b.nama} className="flex justify-between text-slate-600"><dt>{b.nama}</dt><dd>{rupiah(b.jumlah)}</dd></div>
                ))}
            ...
            {konfigurasi.instruksiPembayaran && (
              <div className="rounded-3xl bg-amber-50 p-5 text-[13px] leading-relaxed text-amber-950 ring-1 ring-amber-200/70">
