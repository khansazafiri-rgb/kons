  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-xl font-extrabold text-stone-900">Akun admin</h3>
      ...
      {baru && (
        <div className="grid gap-4 rounded-2xl border border-sewa/30 bg-white p-5 shadow-lembut sm:grid-cols-2">
          <label className="block"><span className="mb-1.5 block text-[12px] font-bold text-stone-600">Nama</span><input value={baru.name} onChange={(e) => setBaru({ ...baru, name: e.target.value })} className={inputCls} /></label>
          <label className="block"><span className="mb-1.5 block text-[12px] font-bold text-stone-600">Email (untuk masuk)</span><input type="email" value={baru.email} onChange={(e) => setBaru({ ...baru, email: e.target.value })} className={inputCls} /></label>
          <label className="block"><span className="mb-1.5 block text-[12px] font-bold text-stone-600">WhatsApp</span><input value={baru.whatsapp} onChange={(e) => setBaru({ ...baru, whatsapp: e.target.value })} className={inputCls} /></label>
          <label className="block">
            <span className="mb-1.5 block text-[12px] font-bold text-stone-600">Kata sandi awal</span>
            <span className="flex gap-2">
              <input value={baru.password} onChange={(e) => setBaru({ ...baru, password: e.target.value })} className={`${inputCls} font-mono`} />
              <button type="button" onClick={() => setBaru({ ...baru, password: sandiAcak() })} className="shrink-0 rounded-xl border border-alba-300 px-3 text-stone-500 hover:text-sewa" title="Buat acak"><KeyRound size={15} /></button>
            </span>
          </label>
          <div className="sm:col-span-2">
            <span className="mb-1.5 block text-[12px] font-bold text-stone-600">Peran</span>
            <div className="grid gap-2 sm:grid-cols-3">
              {Object.entries(PERAN_ADMIN).map(([k, p]) => (
                <button
                  ...
                  className={`rounded-xl border p-3 text-left transition-colors ${baru.role === k ? 'border-sewa bg-sewa/5 ring-1 ring-sewa' : 'border-alba-200 hover:border-alba-300'}`}
                >
                  <p className="text-[13px] font-extrabold text-stone-900">{p.teks}</p>
                  <p className="mt-0.5 text-[11px] leading-snug text-stone-500">{p.ket}</p>
                </button>
              ))}
            </div>
          </div>
          <div className="flex gap-2 sm:col-span-2">
            ...
            <button onClick={() => setBaru(null)} className="rounded-xl px-4 py-2.5 text-[13px] font-semibold text-stone-500 hover:bg-alba-100">Batal</button>
          </div>
        </div>
      )}

      <div className="overflow-hidden rounded-2xl border border-alba-200 bg-white shadow-lembut">
        <table className="w-full text-left text-[13px]">
          <thead className="bg-alba-50 text-[11px] font-bold uppercase tracking-wide text-stone-500">
            <tr>
          ...
          <tbody className="divide-y divide-alba-200">
            {daftar.map((a) => {
              const diriSendiri = saya?.id === a.id;
              return (
                <tr key={a.id} className={a.active ? '' : 'bg-alba-50 text-stone-400'}>
                  <td className="px-4 py-3">
                    <p className="font-bold text-stone-900">{a.name}{diriSendiri && <span className="ml-1.5 text-[11px] font-semibold text-sewa">(kamu)</span>}</p>
                    <p className="text-[12px] text-stone-500">{a.email}{!a.active && ' · nonaktif'}</p>
                  </td>
                  <td className="px-4 py-3">
                    <select
                      value={a.role}
                      ...
                      className="rounded-lg border border-alba-200 bg-white px-2 py-1.5 text-[12px] font-semibold disabled:opacity-60"
                  ...
                  <td className="hidden px-4 py-3 text-stone-500 md:table-cell">{waktu(a.lastLoginAt)}</td>
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-1">
                      <button
                        ...
                        className="grid h-8 w-8 place-items-center rounded-lg text-stone-500 hover:bg-alba-100 hover:text-sewa"
                      >
                        <KeyRound size={14} />
                      </button>
                      {!diriSendiri && (
                      ...
                      {!diriSendiri && (
                        <button
                          ...
                          className="grid h-8 w-8 place-items-center rounded-lg text-stone-500 hover:bg-rose-50 hover:text-rose-600"
                        >
                          <Trash2 size={14} />
                        </button>
                      )}
            ...
            {!daftar.length && (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-stone-500">Belum ada akun admin peminjaman. Buat yang pertama di atas.</td></tr>
            )}
          </tbody>
        </table>
      </div>
