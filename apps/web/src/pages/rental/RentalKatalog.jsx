...
function KartuRuang({ r, tanggal }) {
  return (
    <Link
      ...
      className="group flex flex-col overflow-hidden rounded-3xl bg-white shadow-lembut ring-1 ring-alba-200 transition-shadow hover:shadow-angkat"
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute left-3 top-3 flex flex-wrap gap-1.5">
          {r.kapasitas > 0 && (
            <span className="inline-flex items-center gap-1 rounded-full bg-white/95 px-2.5 py-1 text-[12px] font-bold text-stone-700 shadow-sm">
              <Users size={13} /> {r.kapasitas}
            </span>
          )}
          {r.butuhPenjaga && (
            <span className="rounded-full bg-sewa px-2.5 py-1 text-[11px] font-bold text-white shadow-sm">Dengan penjaga</span>
          )}
        </div>
      </div>
      <div className="flex flex-1 flex-col p-4">
        ...
        {r.fasilitas?.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {r.fasilitas.slice(0, 3).map((f) => (
              <span key={f} className="rounded-lg bg-alba-100 px-2 py-1 text-[11px] font-semibold text-stone-600">{f}</span>
            ))}
            {r.fasilitas.length > 3 && (
              <span className="rounded-lg bg-alba-100 px-2 py-1 text-[11px] font-semibold text-stone-500">+{r.fasilitas.length - 3}</span>
            )}
          </div>
        )}
        <div className="mt-auto flex items-end justify-between pt-4">
          <p className="text-[12px] text-stone-500">
            Mulai<br />
            <span className="text-[18px] font-extrabold text-stone-900">{rupiah(r.harga)}</span>
            <span className="ml-1 text-[12px] text-stone-500">{SATUAN[r.satuan] || ''}</span>
          </p>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <h3 className="mt-0.5 text-[15px] font-extrabold leading-snug text-stone-900">{a.nama}</h3>
        <p className="mt-1 flex items-center gap-1 text-[12px] text-stone-500">
          <Package size={13} /> {a.stok} unit
        </p>
        <p className="mt-auto pt-3 text-[12px] text-stone-500">
          <span className="text-[17px] font-extrabold text-stone-900">{rupiah(a.harga)}</span> {SATUAN[a.satuan] || ''}
        </p>
      </div>
    </Link>
  );
...
export default function RentalKatalog({ tipe = 'RUANG' }) {
  ...
  return (
    <RentalLayout konfigurasi={konfigurasi}>
      <div className="border-b border-alba-200 bg-white">
        <div className="mx-auto max-w-6xl px-4 pb-6 pt-8 sm:px-6">
          <h1 className="text-3xl font-extrabold tracking-tight text-stone-900">{isAlat ? 'Sewa alat medis' : 'Sewa ruang'}</h1>
          <p className="mt-1.5 text-sm text-stone-500">
            {isAlat
              ? 'Pilih alat, tentukan jumlah dan lama pinjamnya.'
              : 'Pilih ruang, lalu lihat jam kosongnya per 30 menit.'}
            {tanggal && <> Tanggal yang dicari: <b className="text-stone-700">{tanggalWib(`${tanggal}T12:00:00+07:00`)}</b>.</>}
          </p>
        </div>
      </div>

      {/* Bilah filter - lengket di bawah header */}
      <div className="sticky top-16 z-20 border-b border-alba-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-2 px-4 py-3 sm:px-6">
          <label className="flex min-w-[200px] flex-1 items-center gap-2 rounded-full bg-alba-100 px-4 py-2.5 focus-within:bg-white focus-within:ring-2 focus-within:ring-sewa">
            <Search size={16} className="text-stone-400" />
            <input
              value={cari}
              onChange={(ev) => ubahParam('q', ev.target.value)}
              placeholder={isAlat ? 'Cari nama alat atau SKU' : 'Cari ruang, lokasi, fasilitas'}
              className="w-full bg-transparent text-sm font-semibold text-stone-900 placeholder:font-normal placeholder:text-stone-400 focus:outline-none"
            />
          </label>

          {!isAlat && (
            <label className="flex items-center gap-2 rounded-full border border-alba-200 bg-white px-4 py-2.5 text-sm">
              ...
              <input
                ...
                className="w-24 bg-transparent font-semibold text-stone-900 placeholder:font-normal placeholder:text-stone-400 focus:outline-none"
              />
            </label>
          )}
            ...
            {['', ...data.kategori].map((k) => (
              <button
                ...
                className={`shrink-0 rounded-full px-4 py-2 text-[13px] font-bold transition-colors ${
                  kategori === k ? 'bg-sewa text-white' : 'bg-alba-100 text-stone-600 hover:bg-alba-200'
                }`}
              >
                {k || 'Semua'}
              </button>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        {!memuat && !galat && (
          <div className="mb-5 flex flex-wrap items-center gap-3 text-sm text-stone-500">
            <span><b className="text-stone-900">{daftar.length}</b> {isAlat ? 'alat' : 'ruang'} ditemukan</span>
            {adaFilter ? (
              <button
                onClick={() => setSp(tanggal ? { tanggal } : {}, { replace: true })}
                className="inline-flex items-center gap-1 rounded-full bg-alba-200 px-3 py-1 text-[12px] font-bold text-stone-600 hover:bg-alba-200"
              >
        ...
        {memuat && (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {[0, 1, 2].map((i) => <div key={i} className="h-80 animate-pulse rounded-3xl bg-alba-200" />)}
          </div>
        )}
        ...
        {!memuat && !galat && daftar.length === 0 && (
          <div className="rounded-3xl border border-dashed border-alba-300 bg-white px-6 py-16 text-center">
